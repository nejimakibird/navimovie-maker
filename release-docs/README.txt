...
NaviMovie-Maker は、GitHub Releases または Vector から入手できます。
...
NaviMovie-Maker は個人開発のアプリであり、現時点ではコード署名を行っていません。
そのため、GitHub Releases や Vector などの公式配布元から入手した場合でも、Windows SmartScreen や Smart App Control により警告が表示される場合があります。

配布ファイルは、GitHub Releases、Vector、または作者Webサイトで案内している公式配布元から取得してください。
...
Simple Mode 表示時は、通常モードで使う不要な操作UIを隠し、キューの状態表示を確認しながら処理できます。
処理中は Simple Mode の「キャンセル」ボタンで中断できます。
ログ表示エリアは初期状態で折りたたまれている場合があります。必要なときに開いて確認してください。


4.4 プレイリストを保存・開く

メニューの「プレイリスト」から、現在の変換キューと主な変換設定を
.nmm-playlist.json ファイルとして保存できます。

ここでのプレイリストは、メディア再生用ではなく、あとで再利用・編集できる
変換ジョブのリストです。読み込むとキューの順序と設定を復元できますが、
完了・失敗・処理中などの実行状態は引き継がれません。

既定の保存先は Videos\NaviMovie-Maker\playlists です。
保存時のローカルファイルが見つからない場合は、項目を残したまま
「対象外」として理由を表示します。
...
表示する出力プリセットは「設定」の「出力形式」タブで変更できます。
少なくとも 1 つの出力プリセットは表示対象として残す必要があります。

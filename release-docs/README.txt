...
- 従来の「ピーク音量」と、ReplayGain 解析を利用する「音量の正規化（dB）」を選択できます。
- 「音量の正規化（dB）」は 89.0 dB を基準とし、80.0～105.0 dB の範囲を 0.1 刻みで指定できます。
- 数値を大きくすると出力音量も大きくなります。ピーク上限を超える場合はリミッターで保護します。
- 大きな値ではリミッターが強く作用し、音の強弱が圧縮される場合があります。
- ReplayGain 解析に失敗した場合は、正規化なしで変換を継続します。
- 通常モードと Simple Mode の両方で利用できます。項目別 loudnorm が有効な場合は項目別設定を優先します。

出力ファイル名:

- 名前衝突がない通常時: 動画タイトル.mp4
- プレイリスト連番が有効な通常時: 001_動画タイトル.mp4
- 管理外ファイルや別ソースの成果物と衝突した場合: 動画タイトル_484306.mp4
- 識別子は実際の衝突時だけ付与し、既存ファイルを上書きしません。
- 過去に識別子付きで保存された追跡済み成果物も引き続き利用できます。
